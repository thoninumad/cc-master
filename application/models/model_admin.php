...
class model_admin extends CI_Model {
    ...
    public function program_eval_viwe($bulan){
		...
		$hasil = $this->db->query("select * from cc_program_eval a JOIN cc_program_input b on a.input_user_c=b.input_user JOIN unit c on c.kode_unit = a.input_user_c  where input_status=5 AND input_bulan = '$bulan'");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
    ...
    public function program_eval_vive($bulan){
		...
		$hasil = $this->db->query("select * from cc_program_eval a JOIN cc_program_input b on a.input_user_c=b.input_user JOIN unit c on c.kode_unit = a.input_user_c  JOIN cc_program_feedback on input_id = fb_id where input_status=1 AND input_bulan = '$bulan'");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
	...
	public function last_unit($kode){
        $hasil = $this->db->query("SELECT kode_unit, kode_lokasi FROM unit WHERE kode_lokasi = '$kode' ORDER BY kode_unit DESC");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
    ...
    public function get_unit_score($unit){
        $hasil = $this->db->query("SELECT sum(input_score) as total_score FROM `cc_program_eval` WHERE input_user_c = '$unit' AND input_status = 1 GROUP BY input_user_c");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
    ...
    public function selectpeval($unit, $program){
        $hasil = $this->db->query("SELECT * FROM `cc_program_eval` WHERE `input_user_c` = '$unit' AND `input_detail_c` = '$program' AND `input_bulan` = '08'");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
    ...
    public function leaderAll(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit where a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score  DESC");
    ...
    public function leaderbym($bulan){
		$havewarrior = $this->db->query("SELECT b.kode_lokasi, b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where a.input_bulan = '$bulan' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score  DESC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
    ...
    public function leaderKC(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KC' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score DESC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
     ...
     public function leaderKCP(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KCP' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score DESC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
     ...
     public function leaderKCS(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KCS' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score DESC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
     ...
     public function leaderKCPS(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KCPS' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score DESC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
     ...
     public function leaderKK(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KK' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score DESC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
     ...
     public function leaderDV(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'Divisi' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score DESC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
     ...
     public function botall(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit where a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score ASC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
    ...
    public function botKC(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KC' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score ASC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
    ...
    public function botKCP(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KCP' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score ASC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
    ...
    public function botKCS(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KCS' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score ASC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
    ...
    public function botDV(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'Divisi' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score ASC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
    ...
    public function botKCPS(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KCPS' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score ASC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
    ...
    public function botKK(){
		$havewarrior = $this->db->query("SELECT b.kode_unit, b.nama_unit, SUM(a.input_score) as total_score FROM `cc_program_eval` a JOIN unit b ON a.input_user_c = b.kode_unit Where b.kode_lokasi = 'KK' AND a.input_status=1 GROUP BY a.input_user_c ORDER BY total_score ASC");
		if($havewarrior->num_rows() > 0){
			return $havewarrior->result();
		}
		else {
	...
	public function update_sesuatu($kode_unit, $program,$data) {
		//Query update from ... where id = ...
		$this->db->where("input_user_c = '$kode_unit' AND input_detail_c = '$program' AND input_bulan = '08'")
				 ->update('cc_program_eval', $data);
	}
	
	public function update_sesuatu22($kode_unit, $program,$data) {
		//Query update from ... where id = ...
		$this->db->where("input_user = '$kode_unit' AND input_detail = '$program'")
				 ->update('cc_program_input', $data);
	}
	...
	public function listuser() {
		//Query mencari record berdasarkan ID
		$hasil = $this->db->query("SELECT a.iduser, a.username, a.password, b.kode_dir, b.nama_unit FROM `user` a JOIN `unit` b on a.username=b.kode_unit WHERE a.role=1 GROUP BY a.iduser ORDER BY a.iduser ASC");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
...
/*		$this->db->insert('tb_penilaian2', $data_penilaian2,"",$rata);*/
	...
	public function add_feedback22($data_feedback) {
		//Quert insert into
		$insert_query= $this->db->insert_string('cc_program_feedback', $data_feedback);
    ...
    public function hitunglate($bulan){
        $year = date("Y");
		$hasil = $this->db->query("SELECT cc_program_eval.input_bulan, COUNT(cc_program_eval.input_id) AS jumlah FROM cc_program_eval where cc_program_eval.input_tanggal > 26  AND cc_program_eval.input_bulan = '$bulan' AND cc_program_eval.input_tahun = '2019' GROUP BY cc_program_eval.input_bulan");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
	...
	public function evalinputbutuh(){
		$hasil = $this->db->query("SELECT * FROM cc_program_eval a JOIN cc_program_input b ON a.input_user_c=b.input_user AND b.input_detail=a.input_detail_c");
		if($hasil->num_rows() > 0){
			return $hasil->result();
		}
		else {
